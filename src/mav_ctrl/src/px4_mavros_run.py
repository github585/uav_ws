...
class Px4Controller:
    ...
    def __init__(self):
        ...
        self.local_vel_sub=rospy.Subscriber(
            "/mavros/local_position/velocity_body", TwistStamped, self.local_vel_callback)
    ...
    def start(self):
        ...
        #self.home_height=self.local_pose.pose.position.z
        #print('[home_height]',self.home_height)
        self.cur_target_pose = self.construct_target(
            0, 0, self.takeoff_height, self.current_heading)
        self.cur_target_vel = self.construct_vel_target(0, 0, self.takeoff_height)

        rate = rospy.Rate(30)    #20->30
        ...
        while not rospy.is_shutdown():
            if(self.mavros_state.mode == 'STABILIZED'):
                break
            elif (self.state is "LAND"  ):
                # if self.local_pose.pose.position.z<self.takeoff_height+0.03:
                # if self.flightModeService(custom_mode='LAND'):
                #     print('set LAND mode ok')
                # continue
                # if(self.local_vel.twist.linear.z<0.01):
                #     # for i in range(100):
                #     #     pass
                #     time.sleep(0.2)
                #     if(self.disarm()):
                ...
                #         self.state = "DISARMED"
                break
            elif (not self.mavros_state.armed and (rospy.Time.now() - last_request > rospy.Duration(2.0))):
                if self.arm():
                    print("Vehicle armed")
                last_request = rospy.Time.now()
            elif (self.mavros_state.mode != 'OFFBOARD' and (rospy.Time.now() - last_request > rospy.Duration(2.0))):
                if self.offboard():
                        print ("Offboard enabled")
                        last_request = rospy.Time.now()

            if self.takeoff_detection() and self.takeoff_check_flag:
                print("Vehicle Took Off!")
                self.takeoff_check_flag=False

            self.isvel=rospy.get_param('/mavros/isvel')
            # print(self.local_vel.twist.linear.z)
    ...
    def construct_target(self, x, y, z, yaw, yaw_rate=1, vx=0, vy=0, vz=0):
        ...
        target_raw_pose.type_mask = PositionTarget.IGNORE_AFX + \
            ...
            PositionTarget.IGNORE_VY+PositionTarget.IGNORE_VZ+PositionTarget.IGNORE_YAW_RATE
    ...
    def set_space_limit(self, x1, x2, y1, y2, z):
        ...
        self.space_limit['y1'] = y1
        self.space_limit['y2'] = y2
