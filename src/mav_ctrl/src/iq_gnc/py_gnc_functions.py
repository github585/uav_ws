...
class gnc_api:
    ...
    def wait4start(self):
        ...
        rospy.loginfo(CYELLOW2 + CBLINK +
                      "Waiting for user to set mode to AUTO" + CEND)
        while not rospy.is_shutdown() and self.current_state_g.mode != "AUTO":
            rospy.sleep(0.5)
            self.set_mode("AUTO")
        else:
            if self.current_state_g.mode == "AUTO":
                rospy.loginfo(
                    CGREEN2 + "Mode set to AUTO. Starting Mission..." + CEND)
                return 0
            else:
