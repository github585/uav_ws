...
class Px4Controller:
    ...
    def start(self):
        ...
        #self.takeoff_height=self.local_pose.pose.position.z
        ...
        print('init',self.cur_target_pose.yaw)
        rate = rospy.Rate(20)
        ...
        while not rospy.is_shutdown():
            if(self.mavros_state.mode == 'STABILIZED'):
                break
            elif (self.state is "LAND") and (self.local_pose.pose.position.z < 0.3):
#                if(self.disarm()):
 #                   self.state = "DISARMED"
                break
            ...
            print(self.cur_target_pose.yaw,self.current_heading,(self.cur_target_pose.yaw-self.current_heading)/self.cur_target_pose.yaw*100)
            if not self.isvel:
                self.local_target_pub.publish(self.cur_target_pose)
            else:
